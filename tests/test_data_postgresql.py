...
This module tests the PostgreSQLAdapter class that provides PostgreSQL connectivity
...
class TestPostgreSQLAdapterInit(unittest.TestCase):
    """Test PostgreSQLAdapter initialization."""

    def test_init_with_defaults(self):
        """
        Test that PostgreSQLAdapter initializes with default connection resolver.

        Verifies:
        - Connection parameters are stored
        - Default connection resolver is psycopg2.connect
        ...
        self.assertTrue(callable(adapter._connection_resolver))
        self.assertEqual(adapter._connection_resolver.__name__, 'connect')
...
class TestPostgreSQLAdapterContextManager(unittest.TestCase):
    """Test PostgreSQLAdapter context manager protocol."""

    @patch('rococo.data.postgresql.psycopg2.connect')
...
class TestPostgreSQLAdapterConnect(unittest.TestCase):
    ...
    def test_connect_property(self, mock_connect):
        ...
        Verifies all connection parameters are passed.
        """
        mock_connection = Mock()
        mock_connect.return_value = mock_connection
...
class TestPostgreSQLAdapterCallCursor(unittest.TestCase):
...
class TestPostgreSQLAdapterBuildCondition(unittest.TestCase):
    ...
    def test_build_condition_bool(self):
        """
        Test _build_condition_string with boolean value.

        Verifies bool condition is created.
        """
        adapter = PostgreSQLAdapter(
            host=TEST_HOST,
            port=TEST_PORT,
...
class TestPostgreSQLAdapterMoveToAudit(unittest.TestCase):
...
class TestPostgreSQLAdapterExecuteQuery(unittest.TestCase):
    ...
    def test_execute_query_select(self):
        ...
        Verifies:
        - Query is executed
        - Results are fetched and mapped to dicts
        """
        adapter = PostgreSQLAdapter(
            host=TEST_HOST,
            port=TEST_PORT,
        ...
        result = adapter.execute_query("SELECT id, name FROM table")

        mock_cursor.execute.assert_called_once_with("SELECT id, name FROM table", {})
    ...
    def test_execute_query_insert(self):
        ...
        result = adapter.execute_query("INSERT INTO table (name) VALUES ('test')")
    ...
    def test_execute_query_with_vars(self):
        ...
        Verifies variables are passed to execute.
        """
        adapter = PostgreSQLAdapter(
            host=TEST_HOST,
            port=TEST_PORT,
        ...
        vars_dict = {'name': 'test'}
        adapter.execute_query("UPDATE table SET name=%(name)s", vars_dict)

        mock_cursor.execute.assert_called_once_with("UPDATE table SET name=%(name)s", vars_dict)


class TestPostgreSQLAdapterRunTransaction(unittest.TestCase):
    ...
    def test_run_transaction_with_tuples(self):
        ...
        - Values are transformed (dict to JSON)
        - Transaction is committed
        """
        adapter = PostgreSQLAdapter(
            host=TEST_HOST,
        ...
        queries = [
            ("INSERT INTO table1 (data) VALUES (%s)", ({'key': 'value'},)),
            ("INSERT INTO table2 (name) VALUES (%s)", ('test',))
        ]
        ...
        self.assertIn('"key"', first_call_args[0][1][0])
        mock_connection.commit.assert_called_once()
...
class TestPostgreSQLAdapterParseResponse(unittest.TestCase):
...
class TestPostgreSQLAdapterGetOne(unittest.TestCase):
    """Test get_one method."""
...
class TestPostgreSQLAdapterGetMany(unittest.TestCase):
...
class TestPostgreSQLAdapterGetCount(unittest.TestCase):
    ...
    def test_get_count_basic(self):
        ...
        mock_cursor = Mock()
        # execute_query returns list of dicts with column values
        mock_cursor.description = [('count',)]
        mock_cursor.fetchall.return_value = [(42,)]  # Returns tuple
        adapter._cursor = mock_cursor
        adapter._connection = Mock()
    ...
    def test_get_count_with_hint_logs_warning(self, mock_logging):
        """
        Test get_count with hint option logs warning.

        PostgreSQL doesn't support generic hints, so it logs.
        """
        adapter = PostgreSQLAdapter(
            host=TEST_HOST,
            port=TEST_PORT,
        ...
        mock_cursor = Mock()
        mock_cursor.description = [('count',)]
        mock_cursor.fetchall.return_value = [(10,)]  # Returns tuple
        adapter._cursor = mock_cursor
        adapter._connection = Mock()

        adapter.get_count(TEST_TABLE, {}, options={'hint': 'some_index'})

        # Verify logging.info was called with hint message
...
class TestPostgreSQLAdapterGetSaveQuery(unittest.TestCase):
    ...
    def test_get_save_query(self):
        ...
        # Verify query structure
        self.assertIn("WITH updated AS", query)
        ...
        # Verify values: update values + unique column + insert values
        # update_values = (1, 'test', 'test@example.com', 1)  # values + first column again
        # insert_values = (1, 'test', 'test@example.com')
        # Total: 7 values
