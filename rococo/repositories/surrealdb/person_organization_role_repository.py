"""PersonOrganizationRole repo class"""
...
class PersonOrganizationRoleRepository(SurrealDbRepository):
    ...
    def find_organizations_by_member(self,
                                     member_id: str,
                                     get_member: bool = False) -> List[PersonOrganizationRole]:
        """finds all the organizations that a Person identified by member_id is a member of 

        Args:
            member_id (str): `person.id`
            get_member (bool, optional): should
                    a member (Person) be fetched too?. Defaults to False.
    ...
    def find_organizations_by_owner(
            self,
            owner_id: str,
            get_owner: bool = False) -> List[PersonOrganizationRole]:
