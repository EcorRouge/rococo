...
class TestVersionedModel(VersionedModel):
    ...
    changed_by_id: Union[UUID, str, None] = dc_field(default_factory=lambda: UUID(int=0))
    
    name: str = None # Example custom field
    related_item_id: Optional[UUID] = dc_field(default=None) # For fetch_related tests
    related_items_ids: Optional[List[UUID]] = dc_field(default_factory=list) # For fetch_related (many)
    ...
    def as_dict(self, convert_datetime_to_iso_string=False, convert_uuids=True):
        ...
        for f_info in fields(self): 
            ...
            if isinstance(val, UUID):
                ...
                data_dict[f_info.name] = [str(item) if convert_uuids else item for item in val]
            elif isinstance(val, datetime.datetime):
                data_dict[f_info.name] = val.isoformat() if convert_datetime_to_iso_string else val
            else:
                data_dict[f_info.name] = val
        ...
        for f_info in fields(cls): 
            if f_info.name in data:
                ...
                is_uuid_type = field_type_actual is UUID or \
                               (hasattr(field_type_actual, '__origin__') and field_type_actual.__origin__ is Union and UUID in getattr(field_type_actual, '__args__', []))
                
                is_datetime_type = field_type_actual is datetime.datetime or \
                                   (hasattr(field_type_actual, '__origin__') and field_type_actual.__origin__ is Union and datetime.datetime in getattr(field_type_actual, '__args__', []))
                ...
                if is_uuid_type:
                    ...
                    if isinstance(val, list) and all(isinstance(item, str) for item in val):
                        ...
                        init_args[f_info.name] = [] # Default to empty list if conversion fails or type mismatch
...
def model_instance(test_user_id):
    instance = TestVersionedModel(
        ...
        changed_on=datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=1),
        version=UUID(int=0),
...
def test_adjust_conditions():
    ...
    adjusted = adjust_conditions(conditions.copy()) # Use copy to avoid modifying original
    assert adjusted["id"] == test_uuid1 # Single UUIDs are not changed by this function
...
def test_get_one_with_uuid_list_in_conditions(repository, mock_adapter, model_instance):
    ...
    mock_adapter.get_one.return_value = None # We are testing conditions adjustment
...
def test_get_many_records(repository, mock_adapter, test_user_id):
    ...
    instance1 = TestVersionedModel(entity_id=UUID(int=1), name="Item 1", version=uuid4(), changed_by_id=test_user_id, changed_on=now, active=True)
    instance2 = TestVersionedModel(entity_id=UUID(int=2), name="Item 2", version=uuid4(), changed_by_id=test_user_id, changed_on=now, active=True)
    ...
    result = repository.get_many(conditions=query_conditions, sort=[('name', 'ASC')], limit=10, offset=0)
...
def test_get_count(repository, mock_adapter):
    query_params = {"name": "Test Category", "active": True} # active is already bool
    # adjust_conditions inside get_count will process query_params
    expected_db_conditions = {'latest': True, 'active': True, "name": "Test Category"} # after merging defaults and query
...
def test_save_new_instance(mock_model_as_dict, mock_model_prepare_for_save, repository, mock_adapter, model_instance, test_user_id):
    ...
    dict_a_from_as_dict = {'entity_id': str(model_instance.entity_id), 'version': str(new_version_uuid), 'changed_on': changed_on_time.isoformat()}
    ...
    mock_adapter.get_move_entity_to_audit_table_query.return_value = ("AUDIT_SQL_PG", (str(model_instance.entity_id),))
    mock_adapter.get_save_query.return_value = ("SAVE_SQL_PG", tuple(expected_data_for_adapter.values()))
    mock_adapter.run_transaction.return_value = True

    result = repository.save(model_instance, send_message=False)

    model_instance.prepare_for_save.assert_called_once_with(changed_by_id=test_user_id)
    
    assert mock_model_as_dict.call_count == 2
    mock_model_as_dict.assert_any_call(convert_datetime_to_iso_string=True) 
    mock_model_as_dict.assert_any_call(convert_datetime_to_iso_string=False, convert_uuids=False)
...
def test_delete_instance(mock_model_as_dict, mock_model_prepare_for_save, repository, mock_adapter, model_instance, test_user_id):
    ...
    dict_a_delete = {'entity_id': str(model_instance.entity_id), 'active': False} # active is False
    dict_b_delete = {
    ...
    mock_adapter.get_move_entity_to_audit_table_query.return_value = ("AUDIT_SQL_DEL_PG", ())
...
def test_save_with_message(mock_model_as_dict, mock_model_prepare_for_save, repository, mock_adapter, mock_message_adapter, model_instance, test_user_id):
    ...
    dict_call_3_for_message = dict_call_1 # Same args, reflects updated instance state

    mock_model_as_dict.side_effect = [dict_call_1, dict_call_2, dict_call_3_for_message]
    ...
    mock_adapter.get_move_entity_to_audit_table_query.return_value = ("AUDIT_SQL_MSG_PG", ())
    ...
    calls[1].assert_called_with(convert_datetime_to_iso_string=False, convert_uuids=False)
    calls[2].assert_called_with(convert_datetime_to_iso_string=True) # BaseRepo message call

    mock_adapter.get_save_query.assert_called_once_with(repository.table_name, expected_data_for_adapter_save)
    mock_adapter.run_transaction.assert_called_once()
...
def test_get_one_with_fetch_related(mock_fetch_related, repository, mock_adapter, model_instance):
    db_data_from_adapter = model_instance.as_dict(convert_uuids=True) # Simpler dict for this
    db_data_from_adapter['changed_on'] = model_instance.changed_on # ensure datetime obj
    mock_adapter.get_one.return_value = db_data_from_adapter
    ...
    mock_fetch_related.assert_called_once_with(ANY, 'related_item_id') # ANY for the instance
...
def test_get_many_with_fetch_related(mock_fetch_related, repository, mock_adapter, model_instance):
    ...
    results = repository.get_many(
        ...
        fetch_related=['related_items_ids'] # Assuming this is a field that holds list of IDs
    )

    assert len(results) == 1
    result_instance = results[0]
    mock_fetch_related.assert_called_once_with(result_instance, 'related_items_ids')
