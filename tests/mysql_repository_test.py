    ...
    def from_dict(cls, data: dict):
        ...
        for f_info in fields(cls): 
            if f_info.name in data:
                ...
                if is_uuid_type or (f_info.name == 'changed_by_id' and is_str_uuid_union_type) : # Special handling for changed_by_id if it can be str
...
def test_save_new_instance(mock_model_as_dict, mock_model_prepare_for_save, repository, mock_adapter, model_instance, test_user_id):
    ...
    mock_adapter.get_move_entity_to_audit_table_query.return_value = ("AUDIT_SQL", (model_instance.entity_id.hex.replace('-', ''),))
    mock_adapter.get_save_query.return_value = ("SAVE_SQL", tuple(expected_data_for_adapter.values()))
    ...
    model_instance.as_dict.assert_called_once_with(convert_datetime_to_iso_string=False, convert_uuids=False)
...
def test_delete_instance(mock_model_as_dict, mock_model_prepare_for_save, repository, mock_adapter, model_instance, test_user_id):
    ...
    def prepare_for_save_side_effect(changed_by_id):
        ...
        model_instance.previous_version = version_before_delete_save # The version before this save
        model_instance.changed_by_id = changed_by_id
    ...
    mock_adapter.get_move_entity_to_audit_table_query.return_value = ("AUDIT_SQL_DEL", ())
    mock_adapter.get_save_query.return_value = ("SAVE_SQL_DEL", ())
    mock_adapter.run_transaction.return_value = True

    result = repository.delete(model_instance) # This sets instance.active = False, then calls save()
...
def test_save_with_message(mock_model_as_dict, mock_model_prepare_for_save, repository, mock_adapter, mock_message_adapter, model_instance, test_user_id):
    ...
    mock_adapter.get_move_entity_to_audit_table_query.return_value = ("AUDIT_SQL_MSG", ())
    ...
    assert mock_model_as_dict.call_count == 2 # One for MySqlRepo, one for BaseRepo message
    calls = mock_model_as_dict.call_args_list
    calls[0].assert_called_with(convert_datetime_to_iso_string=False, convert_uuids=False) # MySqlRepo call
    calls[1].assert_called_with(convert_datetime_to_iso_string=True) # BaseRepo message call
